...
import PopUp.AddMeetingRoomPopUp;
import PopUp.AddMeetingRoomQuestion;
import PopUp.SaveChanges_MeetingRoom;
import PopUp.GlassPaneOverlay;
import PopUp.SuccessAdd_MeetingRoom;
import PopUp.SuccessSaveChanges_MeetingRoom;
 ...
 * @author narag
 */
public class MeetingRoomManagement extends javax.swing.JFrame {
    ...
    public MeetingRoomManagement() {
        ...
        setImageToLabel(mrm_icon, "src/Image/ACTIVE MEETING ROOM.png");
        ...
        setImageToLabel(headerr_icon, "src/Image/ACTIVE MEETING ROOM.png");
        ...
        CustomTableStyler.applyStyle(MRR_table);
        ...
        DefaultTableModel model = (DefaultTableModel) MRR_table.getModel();
        ...
        DefaultTableModel editableModel = new DefaultTableModel(
            ...
            public boolean isCellEditable(int row, int column) {
                return isEditMode && column == 2; // Only make "Status" editable in edit mode
            }
        };

        MRR_table.setModel(editableModel);
        ...
        for (int i = 0; i < MRR_table.getColumnCount(); i++) {
            MRR_table.getColumnModel().getColumn(i).setCellRenderer(cellRenderer);
        }
...
public class RoundedTextField extends JTextField {
    ...
    private void initComponents() {
        ...
        mrr_srch_bx = new RoundedTextField(20);
        ScrollBar = new javax.swing.JScrollPane();
        MRR_table = new javax.swing.JTable();
        MRR_bttn_add_device = new RoundedButton("",30);
        MRR_bttn_update = new RoundedButton("",30);

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        setTitle("QCU RESERVATIONS | Meeting Room Management");
        ...
        mrm_indicator.setBackground(new java.awt.Color(11, 42, 122));
        ...
        mrm_label.setFont(new java.awt.Font("Roboto", 1, 16)); // NOI18N
        mrm_label.setForeground(new java.awt.Color(11, 42, 122));
        ...
        jLabel3.setText("Meeting Room Management");
        ...
        mrr_srch_bx.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                mrr_srch_bxKeyReleased(evt);
            }
        });
        Main_Panel.add(mrr_srch_bx, new org.netbeans.lib.awtextra.AbsoluteConstraints(120, 120, 1000, 40));

        MRR_table.setFont(new java.awt.Font("Roboto", 0, 12)); // NOI18N
        MRR_table.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null},
                {null, null, null},
                {null, null, null},
        ...
        MRR_table.getTableHeader().setReorderingAllowed(false);
        ScrollBar.setViewportView(MRR_table);
        if (MRR_table.getColumnModel().getColumnCount() > 0) {
            MRR_table.getColumnModel().getColumn(0).setResizable(false);
            MRR_table.getColumnModel().getColumn(1).setResizable(false);
            MRR_table.getColumnModel().getColumn(2).setResizable(false);
        }

        Main_Panel.add(ScrollBar, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 190, 1070, 430));

        MRR_bttn_add_device.setBackground(new java.awt.Color(219, 229, 255));
        MRR_bttn_add_device.setFont(new java.awt.Font("Roboto", 1, 18)); // NOI18N
        MRR_bttn_add_device.setText("Add Room");
        MRR_bttn_add_device.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        MRR_bttn_add_device.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MRR_bttn_add_deviceActionPerformed(evt);
            }
        });
        Main_Panel.add(MRR_bttn_add_device, new org.netbeans.lib.awtextra.AbsoluteConstraints(600, 650, 150, 50));

        MRR_bttn_update.setBackground(new java.awt.Color(11, 42, 122));
        MRR_bttn_update.setFont(new java.awt.Font("Roboto", 1, 18)); // NOI18N
        MRR_bttn_update.setForeground(new java.awt.Color(255, 255, 255));
        MRR_bttn_update.setText("Update");
        MRR_bttn_update.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        MRR_bttn_update.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MRR_bttn_updateActionPerformed(evt);
            }
        });
        Main_Panel.add(MRR_bttn_update, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 650, 150, 50));
    ...
    private void MRM_PanelMouseExited(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_MRM_PanelMouseExited
        ...
        mrm_label.setForeground(new Color(11,42,122));
    ...
    private void mrr_srch_bxKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_mrr_srch_bxKeyReleased
        applyFilter();
    }//GEN-LAST:event_mrr_srch_bxKeyReleased

    private void MRR_bttn_updateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MRR_bttn_updateActionPerformed
        if (!isEditMode) {
            // Enable editing mode
            isEditMode = true;
            MRR_bttn_update.setText("Save");

            // Enable combo box for status column
            String[] statuses = {"Available", "Unavailable"};
            JComboBox<String> comboBox = new JComboBox<>(statuses);
            TableColumn statusColumn = MRR_table.getColumnModel().getColumn(2);
            statusColumn.setCellEditor(new DefaultCellEditor(comboBox));

        } else {

            SaveChanges_MeetingRoom SaveMR = new SaveChanges_MeetingRoom();
            GlassPaneOverlay glassPaneOverlay = new GlassPaneOverlay(SaveMR);
            setGlassPane(glassPaneOverlay);
            getGlassPane().setVisible(true);

            // Listen for Yes/No from the popup
            SaveMR.setConfirmListener(new SaveChanges_MeetingRoom.ConfirmListener() {
                @Override
                public void onYes() {
                    saveRoomStatusChanges();
                    isEditMode = false;
                    MRR_bttn_update.setText("Update");
                    getGlassPane().setVisible(false);
                    
                    SuccessSaveChanges_MeetingRoom successPanel = new SuccessSaveChanges_MeetingRoom();
    ...
    }//GEN-LAST:event_MRR_bttn_updateActionPerformed

    private void MRR_bttn_add_deviceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MRR_bttn_add_deviceActionPerformed
        // Open the Add Room popup
        AddMeetingRoomPopUp addRoomPanel = new AddMeetingRoomPopUp();
        GlassPaneOverlay overlay = new GlassPaneOverlay(addRoomPanel);
        setGlassPane(overlay);
        getGlassPane().setVisible(true);

        // Listener inside your popup for Add and Cancel
        addRoomPanel.setAddRoomListener(new AddMeetingRoomPopUp.AddRoomListener() {
            @Override
            public void onAdd(String roomNumber, String status, int capacity) {
                
                getGlassPane().setVisible(false);
                
                AddMeetingRoomQuestion confirmPopup = new AddMeetingRoomQuestion();
                GlassPaneOverlay confirmOverlay = new GlassPaneOverlay(confirmPopup);
                setGlassPane(confirmOverlay);
                getGlassPane().setVisible(true);
                
                confirmPopup.setConfirmListener(new AddMeetingRoomQuestion.ConfirmListener() {
                    @Override
                    public void onYes() {
                        // User confirmed: save to database
                        addRoomToDatabase(roomNumber, status, capacity);
                        getGlassPane().setVisible(false);

                        // Show success popup
                        SuccessAdd_MeetingRoom successPanel = new SuccessAdd_MeetingRoom();
    ...
    }//GEN-LAST:event_MRR_bttn_add_deviceActionPerformed
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(MeetingRoomManagement.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new MeetingRoomManagement().setVisible(true);
            }
        });
    }
    
    public void loadCollabRoomTable() {
        Connection conn = dbConnection.dbCon.getConnection(); // your dbCon class
        DefaultTableModel model = (DefaultTableModel) MRR_table.getModel(); // Replace with your JTable variable
        model.setRowCount(0); // Clear existing rows

        String query = "SELECT RoomNumber, Capacity, Status FROM COLLAB_ROOM";
        ...
        try (PreparedStatement ps = conn.prepareStatement(query);
            ...
            while (rs.next()) {
                ...
                model.addRow(new Object[]{room, capacity, status});
            }
    ...
    public void filterCollabRooms(String searchTerm) {
        Connection conn = dbConnection.dbCon.getConnection();
        DefaultTableModel model = (DefaultTableModel) MRR_table.getModel(); // replace with your JTable
        model.setRowCount(0); // Clear table

        String query = """
            SELECT RoomNumber, Capacity, Status 
            FROM COLLAB_ROOM 
            WHERE RoomNumber LIKE ? OR Status LIKE ? OR CAST(Capacity AS VARCHAR) LIKE ?
        """;
    ...
    private void applyFilter() {
        String keyword = mrr_srch_bx.getText().trim();          // e.g. txtSearch
        filterCollabRooms(keyword);
    }
    
    private void saveRoomStatusChanges() {
        DefaultTableModel model = (DefaultTableModel) MRR_table.getModel();
        Connection conn = dbConnection.dbCon.getConnection();
        ...
        try {
            for (int i = 0; i < model.getRowCount(); i++) {
                ...
                String updateQuery = "UPDATE COLLAB_ROOM SET Status = ? WHERE RoomNumber = ?";
            ...
            loadCollabRoomTable(); // reload data

        } catch (SQLException ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error updating statuses: " + ex.getMessage());
        }
    }
    
    private void addRoomToDatabase(String roomNumber, String status, int capacity) {
        Connection conn = dbConnection.dbCon.getConnection();
        String insertQuery = "INSERT INTO COLLAB_ROOM (RoomNumber, Capacity, Status) VALUES (?, ?, ?)";
        ...
        try (PreparedStatement ps = conn.prepareStatement(insertQuery)) {
            ...
            ps.executeUpdate();
            loadCollabRoomTable(); // Refresh table
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private javax.swing.JButton MRR_bttn_add_device;
    private javax.swing.JButton MRR_bttn_update;
    private javax.swing.JTable MRR_table;
    ...
    private javax.swing.JTextField mrr_srch_bx;
